...
앞의 [선형독립문서](/LA/Linear-Independence.md)에서 basis(basis)의 정의를 간단하게 다루었다. 여기서는 geterating set과 span을 사용해 basis와 basis의 성질을 다룬다. 그리고 차원의 개념을 basis로써 정의해본다.
...
이제 generating set과 span을 사용해 basis를 정의해보자. Generating set은 꼭 서로 선형독립으로 중복되는 정보가 없을 필요는 없다. 예로 2차원 평면의 generating set이 꼭 $\left\{ \begin{bmatrix}1 \\ 0\end{bmatrix}, \begin{bmatrix}0 \\ 1\end{bmatrix} \right\}$일 필요는 없다. $\left\{ \begin{bmatrix}1 \\ 0\end{bmatrix}, \begin{bmatrix}0 \\ 1\end{bmatrix}, \begin{bmatrix}1 \\ 1 \end{bmatrix} \right\}$일 수도 있는 것이다. 따라서 generating set의 정의에 조건을 추가해 basis를 정의할 수 있다. basis는 다음과 같이 정의된다.
...
>
> 벡터공간 $V$의 차원은 $V$의 basis 벡터의 수와 같다. 그리고 이를 $\text{dim}(V)$로 표기한다. $V$의 벡터부분공간 $U \subseteq V$는 $\text{dim}(U) \leqslant \text{dim}(V)$가 성립하며 등호는 벡터공간 $U = V$일때만 성립한다.
...
이번 문서에서는 basis를 중심으로 다루었다. 선형대수학은 벡터공간에서의 변환을 다루게 되는데 벡터공간에서의 기준역할을 해주는 것이 바로 basis이다. 2차원이나 3차원에서 익숙하게 사용하는 좌표계도 암묵적으로 합의한 basis가 있기 때문에 공간내 벡터를 표현하는 수 많은 방식이 있음에도 혼동없이 사용할 수가 있는 것이다. 
...
Basis는 이후에 다루는 개념에서 유기적으로 사용되므로 명확하게 이해하도록 하자.
